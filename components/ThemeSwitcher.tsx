...
export function ThemeSwitcher() {
  ...
  if (!mounted) {
    return (
      <div
        className="fixed right-5 top-5 z-[10000] h-14 w-[7.5rem] rounded-full border border-white/10 bg-white/5 backdrop-blur-xl md:right-8 md:top-8"
        aria-hidden
      />
    )
  }
  ...
  return (
    <motion.div
      ...
      className="fixed right-5 top-5 z-[10000] md:right-8 md:top-8"
    >
          ...
          {themes.map((item) => {
            ...
            return (
              <motion.button
                ...
                className="relative z-10 flex h-11 w-[3.25rem] items-center justify-center rounded-full outline-none transition-colors focus-visible:ring-2 focus-visible:ring-white/50 disabled:cursor-wait disabled:opacity-70"
              >
                <span
                  className={
                    isActive
