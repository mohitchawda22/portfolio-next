...
/** OpenAI blossom mark — not in simple-icons; path from official brand geometry. */
const siChatgpt: SimpleIcon & { viewBox: string } = {
  title: 'ChatGPT',
  slug: 'openai',
  hex: '10A37F',
...
type ToolIcon = SimpleIcon & { viewBox?: string; lightFill?: string }

const toolIcons: Record<AIToolId, ToolIcon> = {
  cursor: { ...siCursor, lightFill: '#EDECEC' },
  'github-copilot': { ...siGithubcopilot, lightFill: '#A371F7' },
  claude: siClaude,
  chatgpt: { ...siChatgpt, lightFill: '#FFFFFF' },
  v0: { ...siV0, lightFill: '#FFFFFF' },
  'figma-ai': siFigma,
  'notion-ai': { ...siNotion, lightFill: '#FFFFFF' },
  linear: siLinear,
}
...
export function AIToolLogo({
  ...
  const brandFill = icon.lightFill ?? `#${icon.hex}`

  return (
    <span
      className={cn(
        'inline-flex h-5 w-5 shrink-0 items-center justify-center sm:h-6 sm:w-6',
        className
      )}
    >
      <BrandMark
